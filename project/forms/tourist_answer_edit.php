...
require(QCUBED_PROJECT_PANEL_DIR . '/TouristAnswerEditPanel.php');

/**
 * This is a draft FormBase object to do Create, Edit, and Delete functionality
 * of the TouristAnswer class.  It uses the code-generated
 * TouristAnswerConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of TouristAnswer columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 */
class TouristAnswerEditForm extends FormBase
{

    /** @var TouristAnswerEditPanel  */
	protected $pnlTouristAnswer;
...
//	protected function formLoad() {}
    ...
    protected function formCreate() {
		...
		$this->pnlTouristAnswer = new TouristAnswerEditPanel($this);
		$intId = Application::instance()->context()->queryStringItem('intId');
	    $this->pnlTouristAnswer->load($intId);
		$this->createButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Confirm(sprintf(t('Are you SURE you want to DELETE this %s?'), t('TouristAnswer'))));
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Ajax('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlTouristAnswer->mctTouristAnswer->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlTouristAnswer->save(true);
            $this->getControl($strControlId)->close();
            $this->redirectToListPage();
        } else { // Refresh
            $this->getControl($strControlId)->close();
            $this->pnlTouristAnswer->refresh(true);
        }
    }
    ...
    */
	protected function redirectToListPage() {
		Application::redirect(QCUBED_FORMS_URL . '/tourist_answer_list.php',
...
// tourist_answer_edit.tpl.php as the included HTML template file
TouristAnswerEditForm::run('TouristAnswerEditForm');
