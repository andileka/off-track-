...
require(QCUBED_PROJECT_PANEL_DIR . '/TrackPointEditPanel.php');

/**
 * This is a draft FormBase object to do Create, Edit, and Delete functionality
 * of the TrackPoint class.  It uses the code-generated
 * TrackPointConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of TrackPoint columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 */
class TrackPointEditForm extends FormBase
{

    /** @var TrackPointEditPanel  */
	protected $pnlTrackPoint;
...
//	protected function formLoad() {}
    ...
    protected function formCreate() {
		...
		$this->pnlTrackPoint = new TrackPointEditPanel($this);
		$intId = Application::instance()->context()->queryStringItem('intId');
	    $this->pnlTrackPoint->load($intId);
		$this->createButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Confirm(sprintf(t('Are you SURE you want to DELETE this %s?'), t('TrackPoint'))));
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Ajax('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlTrackPoint->mctTrackPoint->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlTrackPoint->save(true);
            $this->getControl($strControlId)->close();
            $this->redirectToListPage();
        } else { // Refresh
            $this->getControl($strControlId)->close();
            $this->pnlTrackPoint->refresh(true);
        }
    }
    ...
    */
	protected function redirectToListPage() {
		Application::redirect(QCUBED_FORMS_URL . '/track_point_list.php',
...
// track_point_edit.tpl.php as the included HTML template file
TrackPointEditForm::run('TrackPointEditForm');
