...
require(QCUBED_PROJECT_PANEL_DIR . '/TouristEditPanel.php');

/**
 * This is a draft FormBase object to do Create, Edit, and Delete functionality
 * of the Tourist class.  It uses the code-generated
 * TouristConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Tourist columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 */
class TouristEditForm extends FormBase
{

    /** @var TouristEditPanel  */
	protected $pnlTourist;
...
//	protected function formLoad() {}
    ...
    protected function formCreate() {
		...
		$this->pnlTourist = new TouristEditPanel($this);
		$intId = Application::instance()->context()->queryStringItem('intId');
	    $this->pnlTourist->load($intId);
		$this->createButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Confirm(sprintf(t('Are you SURE you want to DELETE this %s?'), t('Tourist'))));
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Ajax('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlTourist->mctTourist->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlTourist->save(true);
            $this->getControl($strControlId)->close();
            $this->redirectToListPage();
        } else { // Refresh
            $this->getControl($strControlId)->close();
            $this->pnlTourist->refresh(true);
        }
    }
    ...
    */
	protected function redirectToListPage() {
		Application::redirect(QCUBED_FORMS_URL . '/tourist_list.php',
...
// tourist_edit.tpl.php as the included HTML template file
TouristEditForm::run('TouristEditForm');
