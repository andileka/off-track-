...
require(QCUBED_PROJECT_PANEL_DIR . '/EventEditPanel.php');

/**
 * This is a draft FormBase object to do Create, Edit, and Delete functionality
 * of the Event class.  It uses the code-generated
 * EventConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Event columns.
 *
...
//	protected function formLoad() {}
    ...
    protected function formCreate() {
		...
		$this->pnlEvent = new EventEditPanel($this);
		$intId = Application::instance()->context()->queryStringItem('intId');
	    $this->pnlEvent->load($intId);
		$this->createButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Confirm(sprintf(t('Are you SURE you want to DELETE this %s?'), t('Event'))));
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Ajax('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlEvent->mctEvent->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlEvent->save(true);
            $this->getControl($strControlId)->close();
            $this->redirectToListPage();
        } else { // Refresh
            $this->getControl($strControlId)->close();
            $this->pnlEvent->refresh(true);
        }
    }
    ...
    */
	protected function redirectToListPage() {
		Application::redirect(QCUBED_FORMS_URL . '/event_list.php',
