...
require(QCUBED_PROJECT_PANEL_DIR . '/DeviceEditPanel.php');

/**
 * This is a draft FormBase object to do Create, Edit, and Delete functionality
 * of the Device class.  It uses the code-generated
 * DeviceConnector class, which has methods to help with
 * easily creating/defining controls to modify the fields of Device columns.
 *
 * Any display customizations and presentation-tier logic can be implemented
 * here by overriding existing or implementing new methods, properties and variables.
 */
class DeviceEditForm extends FormBase
{
...
//	protected function formLoad() {}
    ...
    protected function formCreate() {
		...
		$this->pnlDevice = new DeviceEditPanel($this);
		$intId = Application::instance()->context()->queryStringItem('intId');
	    $this->pnlDevice->load($intId);
		$this->createButtons();
	}
	...
	protected function CreateButtons() {
		...
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Confirm(sprintf(t('Are you SURE you want to DELETE this %s?'), t('Device'))));
		$this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\Ajax('btnDelete_Click'));
		$this->btnDelete->Visible = $this->pnlDevice->mctDevice->EditMode;
	}
    ...
    */
    protected function dlgOptimisticLocking_ButtonEvent($strFormId, $strControlId, $btn) {
        if ($btn == "Overwrite") {
            $this->pnlDevice->save(true);
            $this->getControl($strControlId)->close();
            $this->redirectToListPage();
        } else { // Refresh
            $this->getControl($strControlId)->close();
            $this->pnlDevice->refresh(true);
        }
    }
    ...
    */
	protected function redirectToListPage() {
		Application::redirect(QCUBED_FORMS_URL . '/device_list.php',
...
// device_edit.tpl.php as the included HTML template file
DeviceEditForm::run('DeviceEditForm');
