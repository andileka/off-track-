 ...
 * subclasses a Position class and can be used to display
 ...
 *
 */
class PositionListGen extends \QCubed\Project\Control\DataGrid
{
	/**
	...
	{
		$this->colId = $this->createNodeColumn("Id", QQN::Position()->Id);
		$this->colLat = $this->createNodeColumn("Lat", QQN::Position()->Lat);
		$this->colLong = $this->createNodeColumn("Long", QQN::Position()->Long);
		$this->colHeight = $this->createNodeColumn("Height", QQN::Position()->Height);
	}
	...
	*/
	public function bindData(QQCondition $objAdditionalCondition = null, $objAdditionalClauses = null) 
	{
		...
		if ($this->Paginator) {
			$this->TotalItemCount = Position::queryCount($objCondition, $objClauses);
		}
		...
		$this->DataSource = Position::queryArray($objCondition, $objClauses);
	}
