 ...
 * subclasses a Hut class and can be used to display
 * a collection of Hut objects.
 *
 ...
 *
 */
class HutListGen extends \QCubed\Project\Control\DataGrid
{
	/**
	...
	{
		$this->colId = $this->createNodeColumn("Id", QQN::Hut()->Id);
		$this->colPosition = $this->createNodeColumn("Position", QQN::Hut()->Position);
		$this->colName = $this->createNodeColumn("Name", QQN::Hut()->Name);
	}
	...
	*/
	public function bindData(QQCondition $objAdditionalCondition = null, $objAdditionalClauses = null) 
	{
		...
		if ($this->Paginator) {
			$this->TotalItemCount = Hut::queryCount($objCondition, $objClauses);
		}
		...
		$this->DataSource = Hut::queryArray($objCondition, $objClauses);
	}
