 ...
 * subclasses a User class and can be used to display
 ...
 *
 */
class UserListGen extends \QCubed\Project\Control\DataGrid
{
	/**
	...
	{
		$this->colId = $this->createNodeColumn("Id", QQN::User()->Id);
		$this->colCompany = $this->createNodeColumn("Company", QQN::User()->Company);
		$this->colEmail = $this->createNodeColumn("Email", QQN::User()->Email);
		$this->colPassword = $this->createNodeColumn("Password", QQN::User()->Password);
		$this->colSalt = $this->createNodeColumn("Salt", QQN::User()->Salt);
	}
	...
	*/
	public function bindData(QQCondition $objAdditionalCondition = null, $objAdditionalClauses = null) 
	{
		...
		if ($this->Paginator) {
			$this->TotalItemCount = User::queryCount($objCondition, $objClauses);
		}
